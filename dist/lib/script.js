 ...
 */
function classifyInputScript(prevoutScriptHex, scriptSigHex, witness) {
 ...
 */
function extractPushedData(scriptSig) {
    ...
    if (opcode >= 0x01 && opcode <= 0x4b) {
        ...
        if (scriptSig.length < 3)
            return null;
        dataLen = scriptSig.readUInt16LE(1);
        dataStart = 3;
    }
    else if (opcode === 0x4e) {
        if (scriptSig.length < 5)
            return null;
    ...
    if (dataStart + dataLen > scriptSig.length)
        return null;
